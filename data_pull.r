...
for (i in 1:(nrow(z))) {
  ...
  mes[i,1] <- c(z$solarrad[i])
}

quality <- data.frame(as.Date(day), mes)
sorted_quality <- quality[order(day),]

plot(sorted_quality, type = "l", xlim = as.Date(c("2019-01-01", "2020-01-01")), main = paste(sitename, "Air Quality", delim = " "), xlab = "date (Jan 2019 to Jan 2020)", ylab = TeX('$Solar Radiation (W/m^{2})$'))
